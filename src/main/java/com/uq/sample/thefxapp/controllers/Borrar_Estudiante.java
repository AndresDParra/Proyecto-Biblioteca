...
public class Borrar_Estudiante {
    ...
    private TextArea Area_Info_Estudiante;
    ...
    public void borrar_Estudiante(ActionEvent actionEvent) {
         if (espacio_nombre_estudiante.getText() == null || espacio_cedula_estudiante.getText().isEmpty()) {
            showAlert(javafx.scene.control.Alert.AlertType.ERROR, "Error", "Codigo must not be empty");
            return;
        }
        for (Estudiante estudiante : Biblioteca.getEstudiantes()) {
            if (estudiante.getNombre().equals(espacio_nombre_estudiante.getText())) {
                Biblioteca.getEstudiantes().remove(estudiante);
                showAlert(javafx.scene.control.Alert.AlertType.INFORMATION, "Success", "Libro eliminado");
                return;
            }
        }
    }
